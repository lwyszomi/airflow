...
TEST_PARENT_METADATA: str = "projects/{}/locations/{}/services/{}/metadataImports"
TEST_NAME_BACKUPS: str = "projects/{}/locations/{}/services/{}/backups/{}"
TEST_NAME_METADATA: str = "projects/{}/locations/{}/services/{}/metadataImports/{}"
...
class TestDataprocMetastoreWithDefaultProjectIdHook(TestCase):
    ...
    def test__get_default_mtls_endpoint(self, mock_client) -> None:
        self.hook._get_default_mtls_endpoint(api_endpoint=TEST_API_ENDPOINT)
        mock_client.assert_called_once_with()
        mock_client.return_value._get_default_mtls_endpoint.assert_called_once_with(
    ...
    def test_create_backup(self, mock_client) -> None:
        ...
        mock_client.return_value.create_backup.assert_called_once_with(
            request=dict(
                parent=TEST_PARENT_SERVICES.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                backup=TEST_BACKUP,
                backup_id=TEST_BACKUP_ID,
                request_id=None,
            ),
    ...
    def test_delete_backup(self, mock_client) -> None:
        ...
        mock_client.return_value.delete_backup.assert_called_once_with(
            request=dict(
                name=TEST_NAME_BACKUPS.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID, TEST_BACKUP_ID),
                request_id=None,
            ),
            metadata=None,
            retry=None,
    ...
    def test_delete_service(self, mock_client) -> None:
        ...
        mock_client.return_value.delete_service.assert_called_once_with(
            request=dict(
                name=TEST_PARENT_SERVICES.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                request_id=None,
            ),
            retry=None,
            timeout=None,
    ...
    def test_export_metadata(self, mock_client) -> None:
        ...
        mock_client.return_value.export_metadata.assert_called_once_with(
            request=dict(
                destination_gcs_folder=TEST_DESTINATION_GCS_FOLDER,
                service=TEST_PARENT_SERVICES.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                request_id=None,
    ...
    @mock.patch(DATAPROC_METASTORE_STRING.format("DataprocMetastoreHook.get_dataproc_metastore_client"))
    def test_get_metadata_import(self, mock_client) -> None:
        self.hook.get_metadata_import(
            project_number=TEST_PROJECT_NUMBER,
            location_id=TEST_LOCATION_ID,
            service_id=TEST_SERVICE_ID,
            import_id=TEST_METADATA_IMPORT_ID,
        )
        mock_client.assert_called_once_with()
        mock_client.return_value.get_metadata_import.assert_called_once_with(
            request=dict(
                name=TEST_NAME_METADATA.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID, TEST_METADATA_IMPORT_ID),
            ),
            metadata=None,
            retry=None,
            timeout=None,
    ...
    def test_list_backups(self, mock_client) -> None:
        ...
        mock_client.return_value.list_backups.assert_called_once_with(
            request=dict(
                parent=TEST_PARENT_BACKUPS.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                page_size=None,
                page_token=None,
                filter=None,
                order_by=None,
    ...
    @mock.patch(DATAPROC_METASTORE_STRING.format("DataprocMetastoreHook.get_dataproc_metastore_client"))
    def test_list_metadata_imports(self, mock_client) -> None:
        ...
        mock_client.assert_called_once_with()
        mock_client.return_value.list_metadata_imports.assert_called_once_with(
            request=dict(
                parent=TEST_PARENT_METADATA.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                page_size=None,
                page_token=None,
                filter=None,
                order_by=None,
    ...
    @mock.patch(DATAPROC_METASTORE_STRING.format("DataprocMetastoreHook.get_dataproc_metastore_client"))
    def test_list_services(self, mock_client) -> None:
        self.hook.list_services(
            project_number=TEST_PROJECT_NUMBER,
            location_id=TEST_LOCATION_ID,
        )
        mock_client.assert_called_once_with()
        mock_client.return_value.list_services.assert_called_once_with(
            request=dict(
                parent=TEST_PARENT.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID),
                page_size=None,
                page_token=None,
    ...
    def test_restore_service(self, mock_client) -> None:
        self.hook.restore_service(
            ...
            location_id=TEST_LOCATION_ID,
            service_id=TEST_SERVICE_ID,
            backup_project_id=TEST_PROJECT_NUMBER,
            backup_location_id=TEST_LOCATION_ID,
        ...
        mock_client.return_value.restore_service.assert_called_once_with(
            request=dict(
                service=TEST_PARENT_SERVICES.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID),
                backup=TEST_NAME_BACKUPS.format(TEST_PROJECT_NUMBER, TEST_LOCATION_ID, TEST_SERVICE_ID, TEST_BACKUP_ID),
                restore_type=None,
                request_id=None,
            ),
            metadata=None,
    ...
    @mock.patch(DATAPROC_METASTORE_STRING.format("DataprocMetastoreHook.get_dataproc_metastore_client"))
    def test_update_metadata_import(self, mock_client) -> None:
        self.hook.update_metadata_import(
            metadata_import=TEST_METADATA_IMPORT,
            update_mask=TEST_UPDATE_MASK,
        )
        mock_client.assert_called_once_with()
        mock_client.return_value.update_metadata_import.assert_called_once_with(
            request=dict(
                metadata_import=TEST_METADATA_IMPORT,
                update_mask=TEST_UPDATE_MASK,
                request_id=None,
