...
class TestDataflowHook(TestCase):
    def setUp(self):
        with mock.patch(
    ...
    def test_create_workspace(self, mock_client):
        self.hook.create_workspace(
            project_id=PROJECT_ID, region=REGION, repository_id=REPOSITORY_ID, workspace_id=WORKSPACE_ID
        )
        ...
        mock_client.return_value.create_workspace.assert_called_once_with(
            request=dict(parent=parent, workspace_id=WORKSPACE_ID),
            retry=DEFAULT,
            timeout=None,
            metadata=(),
        )
