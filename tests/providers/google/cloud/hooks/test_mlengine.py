...
async def test_async_get_job_should_execute_successfully(mocked_link):
    await pytest.hook.get_job(project_id=pytest.PROJECT_ID, job_id=pytest.JOB_ID, session=session)
...
async def test_async_get_job_should_fail_if_no_job_id():
    with pytest.raises(
        ...
        await pytest.hook.get_job(project_id=pytest.PROJECT_ID, job_id=None, session=session)
...
async def test_async_get_job_should_fail_if_no_project_id():
    with pytest.raises(AirflowException, match=r"Google Cloud project id is required."):
        await pytest.hook.get_job(project_id=None, job_id=pytest.JOB_ID, session=session)
...
async def test_async_get_job_status_should_execute_successfully(mocked_get):
    ...
    job_status = await pytest.hook.get_job_status(job_id=pytest.JOB_ID, project_id=pytest.PROJECT_ID)
...
async def test_async_get_job_status_with_exception_should_execute_successfully(mocked_get, caplog):
    ...
    await pytest.hook.get_job_status(job_id=pytest.JOB_ID, project_id=pytest.PROJECT_ID)
...
async def test_async_get_job_status_should_fail_if_no_job_id():
    with pytest.raises(
        ...
        await pytest.hook.get_job_status(project_id=pytest.PROJECT_ID, job_id=None)


@pytest.mark.asyncio
async def test_async_get_job_status_should_fail_if_no_project_id():
    with pytest.raises(AirflowException, match=r"Google Cloud project id is required."):
        await pytest.hook.get_job_status(project_id=None, job_id=pytest.JOB_ID)
