...
async def test_async_create_build_trigger_triggers_on_success_should_execute_successfully(mock_get_build):
    """
    ...
    assert TriggerEvent({"id_": TEST_BUILD_ID, "status": "success", "message": "Build completed"}) == actual


@pytest.mark.asyncio
@mock.patch(CLOUD_BUILD_PATH.format("CloudBuildAsyncClient.get_build"))
async def test_async_create_build_trigger_triggers_on_running_should_execute_successfully(
    mock_get_build, caplog
):
    """
